...
class Attack(ABC):
    ...
    def calculate_loss(self, logits, labels):
        ...
        if self.loss_type == 'CW':
            ...
            loss = -margin.mean()
        elif self.loss_type == 'MarginwithT':
            T = 1
            scaled_logits = logits / T

            target_logit = scaled_logits[np.arange(logits.size(0)), labels.squeeze()]
            # 获取最强干扰类的 logit
            sorted_indices = scaled_logits.argsort(-1)
            best_non_target_class = sorted_indices[sorted_indices != labels[:, None]].reshape(logits.size(0), -1)[:, -1]
            max_other_logit = scaled_logits[np.arange(logits.size(0)), best_non_target_class]

            # 攻击目标：让 max_other_logit 远大于 target_logit
            # loss 增大 -> 攻击变强
            loss = (max_other_logit - target_logit).mean()
