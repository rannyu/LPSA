...
class MaskGAE(nn.Module):
    ...
    def train_step(self, graph: Union[Data, HeteroData], alpha: float = 0.) -> torch.Tensor:
        ...
        # 所有节点类型相同
        # 所有边类型相同
        # 例子：Cora引文网络（只有论文节点，只有引用关系）
        # 2. 异构图 (HeteroData)
        # 多种节点类型
        # 多种边类型
        # 例子：电商网络（用户、商品、店铺等不同节点类型）
    ...
    def train_step_homo(self, graph: Data, alpha: float = 0.) -> torch.Tensor:
        remaining_graph, _ = self.mask(graph)
        x, remaining_edge_index = remaining_graph.x, remaining_graph.edge_index
        masked_edges = remaining_graph.masked_edges         # [2, 3109]
        # x remaining_edge_index [2, 8186]  70%的顶点随机游走走出来的边的id不在这里这是剩下的
        z = self.encoder(x, remaining_edge_index)   # 节点特征矩阵  # 剩余边的图结构
        #     z = [
        # [2708, 1433],  # 第0层: 原始输入特征
        # [2708, 128],   # 第1层: 第一层GNN输出
        left = right = z[-1]
        ...
        if self.degree_decoder is not None and alpha > 0:
            deg = degree(masked_edges[1].flatten(), graph.num_nodes).float()            # 被mask的边的度？每个节点的度数，
            deg = (deg - deg.mean()) / (deg.std() + 1e-6)
            loss += alpha * \
                F.mse_loss(self.degree_decoder(left).squeeze(), deg)    # left.shape [num_nodes, hidden_dim]
        return loss
